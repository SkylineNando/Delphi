...
{$ENDIF}
  DBClient,
  DBTables,
  FireDAC.DatS, FireDAC.Stan.Intf, FireDAC.Stan.Param,
  FireDAC.Phys.Intf,
  FireDAC.DApt.Intf,
  FireDAC.Comp.DataSet, FireDAC.Comp.Client, FireDAC.Stan.Option, FireDAC.Stan.Error;
...
{$ENDIF}
    ...
    procedure LoadAgg;
    procedure LoadAggs;
    procedure SetDS(ADS: TFDMemTable);
    procedure UpdateFilter;
    procedure AssignData(ACDS: TClientDataSet);
    procedure GenerateData;
  public
    { Public declarations }
    procedure LoadMemTable(FileName: String; ACDS: TClientDataSet);
...
procedure TfrmProperties.SetDS(ADS: TFDMemTable);
begin
  FDS := ADS;
  DataSource1.DataSet := ADS;
end;
...
begin
  if ListBox1.ItemIndex > 0 then
    with FDS.Indexes.FindIndex(ListBox1.Items[ListBox1.ItemIndex]) do begin
      ...
      cbxNullFirst.Checked := soNullFirst in Options;
    end
  else begin
    edtName.Text := '';
    edtFields.Text := '';
...
begin
  with TfrmProperties.Create(Application) do begin
    FDMemTable1.CloneCursor(Self.FDMemTable1,
...
begin
  ...
  while not oTable.EOF do begin
    ...
    for i := 0 to oTable.FieldCount - 1 do
      FDMemTable1.Fields[i].Assign(oTable.Fields[i]);
    FDMemTable1.Post;
    oTable.Next;
  end;
end;
...
procedure TfrmProperties.AssignData(ACDS: TClientDataSet);
  ...
  procedure CopyDSData(AADDS: TFDDataSet; ACDS: TClientDataSet);
  var
    i: Integer;
  begin
    ACDS.First;
    while not ACDS.EOF do begin
      ...
      for i := 0 to ACDS.FieldCount - 1 do
        if ACDS.Fields[i].DataType = ftDataSet then
          CopyDSData(TFDDataSet(TDataSetField(AADDS.Fields[i]).NestedDataSet),
...
procedure TfrmProperties.LoadMemTable(FileName: String;
  ACDS: TClientDataSet);
begin
  FDMemTable1.Close;
