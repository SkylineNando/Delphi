...
UnicodeString __fastcall TVersionInfo::BuildStringFileInfo(const UnicodeString& StringName) const
{
  ...
  return L"\\StringFileInfo\\" + FTranslations->Strings[FLanguageCharsetIndex] + L"\\" + StringName;
}
...
void __fastcall TVersionInfo::InitializeFixedFileInfoField(void)
{
  ...
  Win32Check( VerQueryValue(FFileVersionInfoData, L"\\", &Buffer, &BufferLength) );
...
UnicodeString __fastcall TVersionInfo::GetFileDescription(void) const
{
  return QueryStringValue(L"FileDescription");
}
...
UnicodeString __fastcall TVersionInfo::GetProductName(void) const
{
  return QueryStringValue(L"ProductName");
}
...
void __fastcall TVersionInfo::SetFileName(const UnicodeString AFileName)
{
...
UnicodeString __fastcall TVersionInfo::GetAboutBoxString(void) const
{
  UnicodeString AboutString;
  UnicodeString ItemString;
...
UnicodeString __fastcall TVersionInfo::GetCurrentModuleFileName(void)
{
  return Sysutils::GetModuleName((unsigned int)HInstance);
}
