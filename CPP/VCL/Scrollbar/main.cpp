...
void __fastcall TFormMain::Button1Click(TObject *Sender)
{
	...
	if (_wtoi(PosEdit->Text.w_str()))
		ScrollBar1->Position = _wtoi(PosEdit->Text.w_str());
	else
		PosEdit->Text = itoa(ScrollBar1->Position, buf, 10);

	if (_wtoi(MinEdit->Text.w_str()))
		ScrollBar1->Min = _wtoi(MinEdit->Text.w_str());
	else
		MinEdit->Text = itoa(ScrollBar1->Min, buf, 10);

	if (_wtoi(MaxEdit->Text.w_str()))
		ScrollBar1->Max = _wtoi(MaxEdit->Text.w_str());
	else
		MaxEdit->Text = itoa(ScrollBar1->Max, buf, 10);

	if (_wtoi(LargeEdit->Text.w_str()))
		ScrollBar1->LargeChange = (short) _wtoi(LargeEdit->Text.w_str());
	else
		LargeEdit->Text = itoa(ScrollBar1->LargeChange, buf, 10);

	if (_wtoi(SmallEdit->Text.w_str()))
		ScrollBar1->SmallChange = (short) _wtoi(SmallEdit->Text.w_str());
	else
