...
void __fastcall TDockableForm::FormClose(TObject *Sender, TCloseAction &Action)
{
  ...
  if (!Floating && FFloatOnCloseDock)
  {
	// Float when close docked window
	Hide();
	ManualFloat(Types::TRect(FUndockedLeft, FUndockedTop, FUndockedLeft + UndockWidth,
...
void __fastcall TDockableForm::FormStartDock(TObject *Sender, TDragDockObject *&DragObject)

{
  DragObject = new TTransparentDragDockObject(this);
}
//---------------------------------------------------------------------------
