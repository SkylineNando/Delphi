...
void __fastcall TMainForm::ViewToolBar2Execute(TObject *Sender)
{
  ...
  if (ToolBar2->Floating)
  {
    ...
    if (ToolDockForm != NULL)
    {
	  ...
	  if (    !__classid(TToolDockForm)->InheritsFrom( __classid(TCustomForm))
		   || !__classid(TForm)->InheritsFrom( __classid(TCustomForm)) )
...
void __fastcall TMainForm::ToolBar1StartDock(TObject *Sender, TDragDockObject *&DragObject)

{
 if (dynamic_cast<TToolBar*>(Sender) != NULL)
  DragObject = new TTransparentDragDockObject(ToolBar1);
}
//---------------------------------------------------------------------------

void __fastcall TMainForm::LeftDockTabSetTabAdded(TObject *Sender)
{
   const static int cDockTabHeight = 23;
   TDockTabSet * SendingTab = dynamic_cast<TDockTabSet*>(Sender);
   if (SendingTab->Tabs->Count == 1)
   {
	  if ((SendingTab->Align == alBottom ) || (SendingTab->Align == alTop))
	  {
...
void __fastcall TMainForm::LeftDockTabSetTabRemoved(TObject *Sender)
{
  //make the Tabset disappear when there are no tabs
  TDockTabSet * SendingTab = dynamic_cast<TDockTabSet*>(Sender);
  if (SendingTab->Tabs->Count == 0)
	   if ((SendingTab->Align == alBottom ) || (SendingTab->Align == alTop))
			SendingTab->Height = 0;
		else
			SendingTab->Width = 0;
...
void __fastcall TMainForm::LeftDockTabSetGetSiteInfo(TObject *Sender, TControl *DockClient,
		  TRect &InfluenceRect, TPoint &MousePos, bool &CanDock)
{
  TDockTabSet * SendingTab = dynamic_cast<TDockTabSet*>(Sender);
  InfluenceRect.Right = SendingTab->DestinationDockSite->ClientWidth;
  CanDock = (dynamic_cast<TDockableForm*>(DockClient) != NULL) && (SendingTab->Tabs->Count > 0);
}
//---------------------------------------------------------------------------
