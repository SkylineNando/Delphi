...
uses FileCtrl, SvnUIConst, SvnUIUtils{, SvnClientRepoBrowserDialog};
...
begin
  ...
  begin
    aUrl := ValidateURLPath(URLHistory[i]);
    if (CheckoutDialog.URL.Items.IndexOf(aUrl) = -1) then
      CheckoutDialog.URL.Items.Add(aUrl);
  end;
  CheckoutDialog.URL.Text := ValidateURLPath(PathName);
...
procedure TCheckoutDialog.URLExit(Sender: TObject);
begin
  URL.Text := ValidateURLPath(URL.Text);
  EnableOk;
end;
