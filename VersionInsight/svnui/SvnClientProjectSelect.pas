...
{                                                                              }
{******************************************************************************}
unit SvnClientProjectSelect;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, ExtCtrls, ComCtrls, SvnImages;

type
  TSvnProjectSelectDialog = class(TForm)
    Names: TListView;
    Panel1: TPanel;
    Ok: TButton;
    Cancel: TButton;
...
function SelectProject(var FileName: string; const ProjectNames,
  ProjectGroupNames: TStringList): Boolean;
var
  SvnProjectSelectDialog: TSvnProjectSelectDialog;
  I: Integer;
  Item: TListItem;
begin
  Result := False;
  if (ProjectNames.Count = 0) and (ProjectGroupNames.Count = 0) then
    Exit;
  SvnProjectSelectDialog := TSvnProjectSelectDialog.Create(Application);
  try
    ...
    begin
      ...
      Item.ImageIndex := SvnImageModule.GetShellImageIndex(ProjectGroupNames[I]);
    end;
    ...
    begin
      ...
      Item.ImageIndex := SvnImageModule.GetShellImageIndex(ProjectNames[I]);
    end;
...
procedure TSvnProjectSelectDialog.NamesChange(Sender: TObject;
