...
function GetUpdateDialog(const Url: string; AAbortCallBack: TAbortCallBack;
  ...
  AFileRefreshCallBack: TFileRefreshCallBack): TUpdateDialog;
...
function GetUpdateDialog(const Url: string; AAbortCallBack: TAbortCallBack;
  AResolveCallBack: TResolveCallBack; AFileRefreshCallBack: TFileRefreshCallBack): TUpdateDialog;
begin
  Result := TUpdateDialog.Create(Application);
...
procedure TUpdateDialog.AbortClick(Sender: TObject);
begin
  FAbortCallBack;
end;
...
procedure TUpdateDialog.OkClick(Sender: TObject);
var
  I: Integer;
begin
  for I := 0 to Files.Items.Count - 1 do
...
procedure TUpdateDialog.ResolveActionExecute(Sender: TObject);
begin
...
procedure TUpdateDialog.ResolveActionUpdate(Sender: TObject);
begin
  if Assigned(Files.Selected) then
