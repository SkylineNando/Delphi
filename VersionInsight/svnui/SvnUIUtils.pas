...
function ValidateURLPath(const APath: string): string;
...
begin
  Result := APath;
  lowerpath := ansilowercase(APath);
  if (Pos(sHTTP,lowerpath) = 1) then
  begin
    Result := sHTTP + Copy(Result, length(sHTTP)+1, length(Result));
    pslash := length(sHTTP);
  end
  else if (Pos(sHTTPS,lowerpath) = 1) then
  begin
    Result := sHTTPS + Copy(Result, length(sHTTPS)+1, length(Result));
    pslash := length(sHTTPS);
  end
  else
    pslash := 0;

  if (length(Result) > pslash) and (Result[length(Result)] = sSLASH) then
