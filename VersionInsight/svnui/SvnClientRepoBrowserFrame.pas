...
begin
  ...
  FRootNode.Clear;
  URLPath := ValidateURLPath(edURL.Text);
  SvnNode := GetRootNode(URLPath);
  AddSvnToNode(SvnNode, FSvnClient, URLPath, FRevision, 0, 0);
  tvFolders.Items.BeginUpdate;
  FLoading := True;
  try
    tvFolders.Items.Clear;
    AddNodeToTree(FRootNode, nil);
    URLNode := FindURLNode(tvFolders.Items, URLPath);
